(function () {
  const KEY = 'lokanetra-cookie-consent';
  const VERSION = '1';
  let injected = false;
  ...
  function writeConsent(analytics, marketing) {
    try {
      ...
      window.dispatchEvent(new CustomEvent('lokanetra:cookieConsentChanged', { detail: payload }));
    } catch {}
  }
  ...
  window.lokanetraCookieConsent = api;
  window.addEventListener('lokanetra:openCookieConsent', showModal);
