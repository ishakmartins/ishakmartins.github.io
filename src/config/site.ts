export type SiteLink = {
	href: string;
	label: string;
};

export type SiteConfig = {
	name: string;
	title: string;
	description: string;
	siteUrl: string;
	email: string;
	locale: string;
	authorName: string;
	authorRole: string;
	keywords: string[];
	ogImage: string;
	navLinks: SiteLink[];
	extraPages: SiteLink[];
	legalLinks: SiteLink[];
	socialLinks: SiteLink[];
};

const defaultSiteUrl = 'https://open.lokanetra.dev';
const envSiteUrl = process.env.SITE_URL ?? process.env.PUBLIC_SITE_URL;
const normalizedSiteUrl = (envSiteUrl || defaultSiteUrl).replace(/\/+$/, '');

export const siteConfig: SiteConfig = {
	name: 'Lokanetra',
	title: 'Lokanetra | Information Observatory',
	description:
		'A clean Astro theme for UI/UX designer portfolios, case studies, and modern product design presentations.',
	// Set SITE_URL or PUBLIC_SITE_URL to keep canonicals, robots.txt, and the sitemap aligned in each environment.
	siteUrl: normalizedSiteUrl,
	email: 'contact@lokanetra.dev',
	locale: 'en-US',
	authorName: 'Lokanetra',
	authorRole: 'UX Designer',
	keywords: [
		'Astro UI UX portfolio theme',
		'UI UX designer portfolio template',
		'Astro portfolio template',
		'product designer portfolio theme',
		'case study portfolio theme',
	],
	ogImage: '/og-image.svg',
	navLinks: [
		{ href: 'https://lokanetra.dev/', label: 'Homepage' },
	],
	extraPages: [
		{ href: 'https://lokanetra/cookies', label: 'Cookies' },
		{ href: 'https://lokanetra/privacy', label: 'Privacy' },
		{ href: 'https://lokanetra/terms', label: 'Terms' },
	],
	legalLinks: [
		{ href: 'https://lokanetra/cookies', label: 'Cookies' },
		{ href: 'https://lokanetra/privacy', label: 'Privacy' },
		{ href: 'https://lokanetra/terms', label: 'Terms' },
	],
	socialLinks: [
		{ href: 'https://www.lokanetra.dev/', label: 'Homepage' },
		{ href: 'https://www.open.lokanetra.dev/', label: 'Open Products' },
	],
};
